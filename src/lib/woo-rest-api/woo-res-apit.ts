...
import {
    ...
    WooRestApiEndpoint,
    WooRestApiMethod,
} from './typesANDinterfaces' // Typescript types for the library
...
export type {
    ...
    WooRestApiEndpoint,
    WooRestApiMethod,
} from './typesANDinterfaces' // Export all the types
...
export default class WooCommerceRestApi<T extends WooRestApiOptions> {
    ...
    _normalizeQueryString(url: string, params: Partial<Record<string, any>>): string {
        ...
        for (const i in values) {
             ...
             */
            queryString +=
                encodeURIComponent(values[i]) + '=' + encodeURIComponent(<string | number | boolean>query[values[i]])
        }
        /**
     ...
     * @return {Object}
     */
    async postOrderOnLanguage<T extends WooRestApiEndpoint>(
        lang: string,
        endpoint: T,
        data: Record<string, unknown>,
        params?: Partial<WooRestApiParams>,
